...
from tools import compare_phase10_24b
...
def _run(tmp_path: Path, modern_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return compare_phase10_24b.run_comparison(
        argparse.Namespace(
...
def test_compare_phase10_24b_help_exposes_required_arguments() -> None:
    help_text = compare_phase10_24b.build_parser().format_help()
...
def test_compare_phase10_24b_classifies_effective_timeseries_case(
    tmp_path: Path,
) -> None:
    metadata = _run(
        tmp_path,
    ...
    assert metadata["classification"] == "SIGMA_THETA_TIMESERIES_EFFECTIVE"
    ...
    assert (tmp_path / "out" / "phase10_24b_summary.csv").exists()
    assert (tmp_path / "out" / "phase10_24b_metadata.json").exists()


def test_compare_phase10_24b_detects_pressure_ok_opening_shifted(
    tmp_path: Path,
) -> None:
    metadata = _run(
        tmp_path,
...
def test_compare_phase10_24b_detects_opening_ok_pressure_shifted(
    tmp_path: Path,
) -> None:
    metadata = _run(
        tmp_path,
...
def test_compare_phase10_24b_detects_no_improvement(tmp_path: Path) -> None:
    metadata = _run(
        tmp_path,
        _modern_rows(
    ...
    assert metadata["classification"] == "SIGMA_THETA_TIMESERIES_NO_IMPROVEMENT"


def test_compare_phase10_24b_rejects_missing_inputs(tmp_path: Path) -> None:
    modern_csv = tmp_path / "modern.csv"
    _write_csv(modern_csv, _modern_rows(opening_time_s=510.0, sink_time_s=540.0, max_pressure_Pa=69040000.0))

    try:
        compare_phase10_24b.run_comparison(
            argparse.Namespace(
