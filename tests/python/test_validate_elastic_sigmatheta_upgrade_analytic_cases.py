...
from tools.validate_elastic_sigmatheta_upgrade_analytic_cases import (
    SOURCE,
    VALID_STATUS,
    main,
    validate,
...
def test_help(monkeypatch, capsys) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["tools/validate_elastic_sigmatheta_upgrade_analytic_cases.py", "--help"],
    )
    try:
        main()
...
def test_wrong_source_is_partial(tmp_path: Path) -> None:
    data = json.loads(CASES_PATH.read_text(encoding="utf-8"))
    data["source"] = "ELASTIC_INITIAL_WELLBORE_STATE"
