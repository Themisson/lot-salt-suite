...
TEST_CASE("AxisymQ9: linear displacement patch is recovered from consistent nodal forces",
          "[patch][q9]") {
    AxisymQ9 elem;
    ElasticIsotropic model(25.0e9, 0.30);
    Mesh2D mesh = build_q9_mesh(1.0, 2.0, 1.0, 1, 1);
...
TEST_CASE("AxisymQ9: B matrix reproduces quadratic field gradients at Gauss points",
          "[patch][q9]") {
    AxisymQ9 elem;
    Mesh2D mesh = build_q9_mesh(1.0, 2.0, 1.0, 1, 1);
