...
TEST_CASE("Lame T3: radial displacement converges at linear-element rate",
          "[lame][t3][convergence]") {
    ...
    for (int n_radial : meshes) {
        const int n_axial = n_radial;
        Mesh2D mesh = build_t3_mesh(Ri, Re, height, n_radial, n_axial);
        auto K = Assembler::assemble_K(mesh, elem, model);
        auto f = assemble_inner_pressure_t3(mesh, elem, n_radial, n_axial, p_inner);
