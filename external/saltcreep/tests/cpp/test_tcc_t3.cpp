...
TEST_CASE("TCC Modelo A T3: closure remains compatible with L3 oracle",
          "[tcc][t3][regression]") {
    fs::path case_path;
    ...
    Mesh2D mesh = build_t3_mesh(Ri, Re, height, n_radial, n_axial, cd.mesh.ratio);
    auto K = Assembler::assemble_K(mesh, elem, model);
    auto f = assemble_inner_pressure_t3(mesh, elem, n_radial, n_axial, cd.fluid_Pa);
    ...
    fs::path tmp = fs::temp_directory_path() / "saltcreep_test" / "modelo_A_T3";
