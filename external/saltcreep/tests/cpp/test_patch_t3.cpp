...
TEST_CASE("AxisymT3: linear displacement patch is recovered from consistent nodal forces",
          "[patch][t3]") {
    AxisymT3 elem;
    ElasticIsotropic model(25.0e9, 0.30);
    Mesh2D mesh = build_t3_mesh(1.0, 2.0, 1.0, 1, 1);
...
TEST_CASE("AxisymT3: B matrix reproduces linear field gradients at centroid",
          "[patch][t3]") {
    AxisymT3 elem;
    Mesh2D mesh = build_t3_mesh(1.0, 2.0, 1.0, 1, 1);
    for (int e = 0; e < mesh.n_elements; ++e) {
        Eigen::VectorXd ue(6);
        auto coords = element_coords(mesh, e, elem.n_nodes());
        for (int i = 0; i < 3; ++i) {
            ue[2 * i] = 1.0 + 0.1 * coords[i].r + 0.05 * coords[i].z;
            ue[2 * i + 1] = -0.05 * coords[i].r + 0.2 * coords[i].z;
        }

        const auto gp = elem.gauss_points()[0];
        double N[3];
        ...
        const double u_r_gp = 1.0 + 0.1 * r_gp + 0.05 * z_gp;
