...
TEST_CASE("AxisymT6: linear displacement patch is recovered from consistent nodal forces",
          "[patch][t6]") {
    AxisymT6 elem;
    ElasticIsotropic model(25.0e9, 0.30);
    Mesh2D mesh = build_t6_mesh(1.0, 2.0, 1.0, 1, 1);
    ...
    bc[mesh.dof_index(2, 1)] = u_expected[mesh.dof_index(2, 1)];

    Eigen::VectorXd u = solve_with_dirichlet(K, f, bc);
    REQUIRE((u - u_expected).norm() / u_expected.norm() < 1.0e-13);
}

TEST_CASE("AxisymT6: B matrix reproduces quadratic field gradients at Gauss points",
          "[patch][t6]") {
    AxisymT6 elem;
    Mesh2D mesh = build_t6_mesh(1.0, 2.0, 1.0, 1, 1);

    for (int elem_id = 0; elem_id < mesh.n_elements; ++elem_id) {
