...
TEST_CASE("TCC Modelo A AQ9: closure remains compatible with L3 oracle",
          "[tcc][aq9][regression]") {
    fs::path case_path;
    ...
    Mesh2D mesh = build_aq9_mesh(Ri, Re, height, n_radial, n_axial, cd.mesh.ratio);
    auto K = Assembler::assemble_K(mesh, elem, model);
    auto f = assemble_inner_pressure_aq9(mesh, elem, n_radial, n_axial, cd.fluid_Pa);
    ...
    fs::path tmp = fs::temp_directory_path() / "saltcreep_test" / "modelo_A_AQ9";
