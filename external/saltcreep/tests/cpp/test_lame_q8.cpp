...
TEST_CASE("Lame Q8: radial displacement converges at quadratic-element rate",
          "[lame][q8][convergence]") {
    ...
    for (int n_radial : meshes) {
        ...
        Mesh2D mesh = build_q8_mesh(Ri, Re, height, n_radial, n_axial);
        auto K = Assembler::assemble_K(mesh, elem, model);
        auto f = assemble_inner_pressure_q8(mesh, elem, n_radial, n_axial, p_inner);
