...
TEST_CASE("TCC Modelo A Q8: closure remains compatible with L3 oracle",
          "[tcc][q8][regression]") {
    fs::path case_path;
    ...
    Mesh2D mesh = build_q8_mesh(Ri, Re, height, n_radial, n_axial, cd.mesh.ratio);
    ...
    fs::path tmp = fs::temp_directory_path() / "saltcreep_test" / "modelo_A_Q8";
